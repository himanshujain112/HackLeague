...
"Your goal is to ensure accurate validation, fair competition, and helpful AI-driven feedback while keeping responses engaging."
    ...
    try:
        ...
        # Log request details
        logging.info(f"[{request_timestamp}] Request ID {request_id} - Response for {user_id}: {complete_text}")
        ...
        logging.error(f"Error for {user_id} (Request ID: {request_id}) at [{request_timestamp}]: {e}")
        return "⚠️ AI validation failed. Please try again later!"
